...
public class CausticsFilter extends WholeImageFilter {
   ...
   protected int[] filterPixels(int width, int height, int[] inPixels, Rectangle transformedSpace) {
      ...
      int srcWidth = super.originalSpace.width;
      int srcHeight = super.originalSpace.height;
            ...
            for(int s = 0; s < this.samples; ++s) {
               ...
               if (this.dispersion > 0.0F) {
                  for(int c = 0; c < 3; ++c) {
                     ...
                     if (!(srcX < 0.0F) && !(srcX >= (float)(outWidth - 1)) && !(srcY < 0.0F) && !(srcY >= (float)(outHeight - 1))) {
   ...
   protected float evaluate(float x, float y) {
      ...
      float f = (double)this.turbulence == (double)0.0F ? Noise.noise3(xt, y, tt) : turbulence2(xt, y, tt, this.turbulence);
      return f;
   }
