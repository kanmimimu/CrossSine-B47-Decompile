...
public class PlasmaFilter extends WholeImageFilter implements Serializable {
   ...
   private int randomRGB(int[] inPixels, int x, int y) {
      if (this.useImageColors) {
         return inPixels[y * super.originalSpace.width + x];
      } else {
   ...
   private int displace(int rgb, float amount) {
      ...
      int g = rgb >> 8 & 255;
      int b = rgb & 255;
      r = PixelUtils.clamp(r + (int)((double)amount * ((double)this.randomGenerator.nextFloat() - (double)0.5F)));
      g = PixelUtils.clamp(g + (int)((double)amount * ((double)this.randomGenerator.nextFloat() - (double)0.5F)));
      b = PixelUtils.clamp(b + (int)((double)amount * ((double)this.randomGenerator.nextFloat() - (double)0.5F)));
      return -16777216 | r << 16 | g << 8 | b;
   }
      ...
      if (this.useColormap && this.colormap != null) {
         ...
         for(int y = 0; y < height; ++y) {
            for(int x = 0; x < width; ++x) {
               outPixels[index] = this.colormap.getColor((float)(outPixels[index] & 255) / 255.0F);
               ++index;
            }
         }
      }
