...
public class ShapeFilter extends WholeImageFilter {
   ...
   private static final int sqrt2 = (int)((double)41.0F * Math.sqrt((double)2.0F));
   private static final int sqrt5 = (int)((double)41.0F * Math.sqrt((double)5.0F));
   ...
   private void makeMap(int[] pixels, int[] map, int width, int height) {
      for(int y = 0; y < height; ++y) {
         for(int x = 0; x < width; ++x) {
            int offset = x + y * width;
            int b = this.useAlpha ? pixels[offset] >> 24 & 255 : PixelUtils.brightness(pixels[offset]);
            map[offset] = b * 41 / 10;
         }
      }
      ...
      for(int y = 0; y < height; ++y) {
         for(int x = 0; x < width; ++x) {
            ...
            if (m == 0) {
               ...
               sa = this.useAlpha ? pixels[offset] >> 24 & 255 : PixelUtils.brightness(pixels[offset]);
               if (this.invert) {
                  sr = 255 - sr;
                  sg = 255 - sg;
                  sb = 255 - sb;
