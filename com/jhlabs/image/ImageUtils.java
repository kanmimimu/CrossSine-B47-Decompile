...
public abstract class ImageUtils {
   ...
   public static BufferedImage createImage(ImageProducer producer) {
      PixelGrabber pg = new PixelGrabber(producer, 0, 0, -1, -1, (int[])null, 0, 0);
   ...
   public static BufferedImage convertImageToARGB(Image image) {
      if (image instanceof BufferedImage && ((BufferedImage)image).getType() == 2) {
         ...
         BufferedImage p = new BufferedImage(image.getWidth((ImageObserver)null), image.getHeight((ImageObserver)null), 2);
         Graphics2D g = p.createGraphics();
         ((Graphics)g).drawImage(image, 0, 0, (ImageObserver)null);
         ((Graphics)g).dispose();
         return p;
      }
   }
