   ...
   public BufferedImage filter(BufferedImage src, BufferedImage dst) {
      ...
      if (this.mask == null) {
         ...
         for(int y = 0; y < height; ++y) {
            ((AbstractBufferedImageOp)this).getRGB(src, 0, y, width, 1, inPixels);
            ((AbstractBufferedImageOp)this).getRGB(this.mask, 0, y % maskHeight, maskWidth, 1, maskPixels);
            ...
            for(int x = 0; x < width; ++x) {
               ...
               float f = ImageMath.smoothStep((float)iv - s, (float)iv + s, (float)v);
               int a = (int)(255.0F * f);
               if (this.invert) {
                  a = 255 - a;
               }

               inPixels[x] = inRGB & -16777216 | a << 16 | a << 8 | a;
            }
