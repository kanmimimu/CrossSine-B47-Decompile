...
public class Gradient extends ArrayColormap implements Cloneable, Serializable {
   ...
   public Gradient(int[] rgb) {
      this((int[])null, rgb, (byte[])null);
   }

   public Gradient(int[] x, int[] rgb) {
      this(x, rgb, (byte[])null);
   }
   ...
   public void setColor(int n, int color) {
      int firstColor = super.map[0];
      int lastColor = super.map[255];
      if (n > 0) {
         for(int i = 0; i < n; ++i) {
            super.map[i] = ImageMath.mixColors((float)i / (float)n, firstColor, color);
         }
      }

      if (n < 255) {
         for(int i = n; i < 256; ++i) {
            super.map[i] = ImageMath.mixColors((float)(i - n) / (float)(256 - n), color, lastColor);
         }
      }
   ...
   public byte getKnotBlend(int n) {
      return (byte)(this.knotTypes[n] & 112);
   }
   ...
   public void splitSpan(int n) {
      ...
      this.addKnot(x, ((ArrayColormap)this).getColor((float)x / 256.0F), this.knotTypes[n]);
      this.rebuildGradient();
   }
   ...
   private void rebuildGradient() {
      ...
      for(int i = 1; i < this.numKnots - 1; ++i) {
         ...
         for(int j = this.xKnots[i]; j < end; ++j) {
            ...
            float[] hsb1 = Color.RGBtoHSB(rgb1 >> 16 & 255, rgb1 >> 8 & 255, rgb1 & 255, (float[])null);
            float[] hsb2 = Color.RGBtoHSB(rgb2 >> 16 & 255, rgb2 >> 8 & 255, rgb2 & 255, (float[])null);
            float t = (float)(j - this.xKnots[i]) / spanLength;
            int type = this.getKnotType(i);
            int blend = this.getKnotBlend(i);
            if (j >= 0 && j <= 255) {
               switch (blend) {
                  case 16:
                  default:
               ...
               switch (type) {
                  case 0:
                     super.map[j] = ImageMath.mixColors(t, rgb1, rgb2);
                     break;
                  case 1:
                  case 2:
                     if (type == 1) {
                     ...
                     float h = ImageMath.lerp(t, hsb1[0], hsb2[0]) % ((float)Math.PI * 2F);
                     float s = ImageMath.lerp(t, hsb1[1], hsb2[1]);
                     float b = ImageMath.lerp(t, hsb1[2], hsb2[2]);
                     super.map[j] = -16777216 | Color.HSBtoRGB(h, s, b);
               }
            }
         }
      }
   ...
   public void randomize() {
      this.numKnots = 4 + (int)((double)6.0F * Math.random());
      this.xKnots = new int[this.numKnots];
      this.yKnots = new int[this.numKnots];
      this.knotTypes = new byte[this.numKnots];

      for(int i = 0; i < this.numKnots; ++i) {
         this.xKnots[i] = (int)((double)255.0F * Math.random());
         this.yKnots[i] = -16777216 | (int)((double)255.0F * Math.random()) << 16 | (int)((double)255.0F * Math.random()) << 8 | (int)((double)255.0F * Math.random());
         this.knotTypes[i] = 32;
      }
   ...
   public void mutate(float amount) {
      for(int i = 0; i < this.numKnots; ++i) {
         ...
         int g = rgb >> 8 & 255;
         int b = rgb & 255;
         r = PixelUtils.clamp((int)((double)r + (double)(amount * 255.0F) * (Math.random() - (double)0.5F)));
         g = PixelUtils.clamp((int)((double)g + (double)(amount * 255.0F) * (Math.random() - (double)0.5F)));
         b = PixelUtils.clamp((int)((double)b + (double)(amount * 255.0F) * (Math.random() - (double)0.5F)));
         this.yKnots[i] = -16777216 | r << 16 | g << 8 | b;
         this.knotTypes[i] = 32;
      }
