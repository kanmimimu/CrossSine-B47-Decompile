...
public class PerspectiveFilter extends TransformFilter {
   ...
   protected void transformInverse(int x, int y, float[] out) {
      out[0] = (float)super.originalSpace.width * (this.A * (float)x + this.B * (float)y + this.C) / (this.G * (float)x + this.H * (float)y + this.I);
      out[1] = (float)super.originalSpace.height * (this.D * (float)x + this.E * (float)y + this.F) / (this.G * (float)x + this.H * (float)y + this.I);
   }
