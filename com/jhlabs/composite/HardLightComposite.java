   ...
   static class Context extends RGBComposite.RGBCompositeContext {
      ...
      public void composeRGB(int[] src, int[] dst, float alpha) {
         ...
         for(int i = 0; i < w; i += 4) {
            ...
            int dor;
            if (sr > 127) {
               dor = 255 - 2 * RGBComposite.RGBCompositeContext.multiply255(255 - sr, 255 - dir);
            } else {
               dor = 2 * RGBComposite.RGBCompositeContext.multiply255(sr, dir);
            }

            int dog;
            if (sg > 127) {
               dog = 255 - 2 * RGBComposite.RGBCompositeContext.multiply255(255 - sg, 255 - dig);
            } else {
               dog = 2 * RGBComposite.RGBCompositeContext.multiply255(sg, dig);
            }

            int dob;
            if (sb > 127) {
               dob = 255 - 2 * RGBComposite.RGBCompositeContext.multiply255(255 - sb, 255 - dib);
            } else {
               dob = 2 * RGBComposite.RGBCompositeContext.multiply255(sb, dib);
            }
