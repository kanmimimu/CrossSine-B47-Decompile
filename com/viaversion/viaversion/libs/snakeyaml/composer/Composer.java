...
public class Composer {
   ...
   protected Node composeScalarNode(String anchor, List blockComments) {
      ...
      if (tag != null && !tag.equals("!")) {
         ...
         if (nodeTag.isCustomGlobal() && !this.loadingConfig.getTagInspector().isGlobalTagAllowed(nodeTag)) {
            throw new ComposerException((String)null, (Mark)null, "Global tag is not allowed: " + tag, ev.getStartMark());
         }
      } else {
   ...
   protected Node composeSequenceNode(String anchor) {
      ...
      if (tag != null && !tag.equals("!")) {
         nodeTag = new Tag(tag);
         if (nodeTag.isCustomGlobal() && !this.loadingConfig.getTagInspector().isGlobalTagAllowed(nodeTag)) {
            throw new ComposerException((String)null, (Mark)null, "Global tag is not allowed: " + tag, startEvent.getStartMark());
         }
      } else {
