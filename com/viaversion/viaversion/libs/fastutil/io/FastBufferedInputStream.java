...
public class FastBufferedInputStream extends MeasurableInputStream implements RepositionableStream {
   ...
   public static final EnumSet ALL_TERMINATORS = EnumSet.allOf(LineTerminator.class);
   ...
   public int readLine(byte[] array, EnumSet terminators) throws IOException {
   ...
   public int readLine(byte[] array, int off, int len, EnumSet terminators) throws IOException {
   ...
   private long skipByReading(long n) throws IOException {
      long toSkip;
      int len;
      for(toSkip = n; toSkip > 0L; toSkip -= (long)len) {
         len = this.is.read(this.buffer, 0, (int)Math.min((long)this.buffer.length, toSkip));
         if (len <= 0) {
            break;
         }
      }
   ...
   public static enum LineTerminator {
      ...
      // $FF: synthetic method
      private static LineTerminator[] $values() {
         return new LineTerminator[]{CR, LF, CR_LF};
      }
   }
}
