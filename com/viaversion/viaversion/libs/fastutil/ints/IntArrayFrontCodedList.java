...
public class IntArrayFrontCodedList extends AbstractObjectList implements Serializable, Cloneable, RandomAccess {
      ...
      if (ratio < 1) {
         ...
         for(int b = 0; arrays.hasNext(); ++n) {
            ...
            if (n % ratio == 0) {
               ...
               BigArrays.copyToBig((int[])a[b], 0, (int[][])array, curSize, (long)length);
               curSize += (long)length;
            } else {
   ...
   private int extract(int index, int[] a, int offset, int length) {
      ...
      if (delta == 0) {
         ...
         if (currLen < length) {
            BigArrays.copyFromBig(this.array, pos + (long)count(arrayLength) + (long)count(common), a, currLen + offset, Math.min(arrayLength, length - currLen));
         }
         ...
         public int[] next() {
            if (!this.hasNext()) {
               ...
               if (this.i % IntArrayFrontCodedList.this.ratio == 0) {
                  ...
                  BigArrays.copyFromBig((int[][])IntArrayFrontCodedList.this.array, this.pos + (long)IntArrayFrontCodedList.count(length), (int[])this.s, 0, length);
