...
public class IntLinkedOpenHashSet extends AbstractIntSortedSet implements Serializable, Cloneable, Hash {
   ...
   public IntLinkedOpenHashSet(Collection c, float f) {
      this(c.size(), f);
      this.addAll(c);
   }

   public IntLinkedOpenHashSet(Collection c) {
      this(c, 0.75F);
   }
   ...
   public static IntLinkedOpenHashSet toSet(IntStream stream) {
      return (IntLinkedOpenHashSet)stream.collect(IntLinkedOpenHashSet::new, IntLinkedOpenHashSet::add, IntLinkedOpenHashSet::addAll);
   }

   public static IntLinkedOpenHashSet toSetWithExpectedSize(IntStream stream, int expectedSize) {
      return expectedSize <= 16 ? toSet(stream) : (IntLinkedOpenHashSet)stream.collect(new IntCollections.SizeDecreasingSupplier(expectedSize, (size) -> size <= 16 ? new IntLinkedOpenHashSet() : new IntLinkedOpenHashSet(size)), IntLinkedOpenHashSet::add, IntLinkedOpenHashSet::addAll);
   }
   ...
   public IntSpliterator spliterator() {
      return IntSpliterators.asSpliterator(this.iterator(), Size64.sizeOf((Collection)this), 337);
   }
   ...
   private final class SetIterator implements IntListIterator {
      ...
      public void forEachRemaining(java.util.function.IntConsumer action) {
         ...
         for(long[] link = IntLinkedOpenHashSet.this.link; this.next != -1; action.accept(key[this.curr])) {
            this.curr = this.next;
      ...
      public void remove() {
         ...
         if (this.curr == -1) {
            ...
            if (this.prev == -1) {
               ...
               long[] var6 = IntLinkedOpenHashSet.this.link;
            ...
            if (this.next == -1) {
               ...
               long[] var7 = IntLinkedOpenHashSet.this.link;
               int var8 = this.next;
               var7[var8] ^= (IntLinkedOpenHashSet.this.link[this.next] ^ ((long)this.prev & 4294967295L) << 32) & -4294967296L;
            }
