...
public abstract class ViaCommandHandler implements ViaVersionCommand {
   ...
   public void registerSubCommand(ViaSubCommand command) {
      boolean var10000 = command.name().matches("^[a-z0-9_-]{3,15}$");
      String var4 = command.name();
      Preconditions.checkArgument(var10000, var4 + " is not a valid sub-command name.");
      var10000 = !this.hasSubCommand(command.name());
      String var6 = command.name();
      Preconditions.checkArgument(var10000, "ViaSubCommand " + var6 + " does already exists!");
   ...
   public boolean onCommand(ViaCommandSender sender, String[] args) {
      ...
      if (!hasPermissions) {
         ...
         if (!this.hasPermission(sender, handler.permission())) {
            ...
            if (!result) {
               String var8 = handler.usage();
               sender.sendMessage("Usage: /viaversion " + var8);
            }
   ...
   public void showHelp(ViaCommandSender sender) {
      ...
      if (allowed.isEmpty()) {
         ...
         String var6 = Via.getPlatform().getPluginVersion();
         sender.sendMessage(ViaSubCommand.color("&aViaVersion &c" + var6));
