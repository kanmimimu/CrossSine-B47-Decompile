...
public class BukkitViaInjector extends LegacyViaInjector {
   ...
   public ProtocolVersion getServerProtocolVersion() throws ReflectiveOperationException {
      return PaperViaInjector.PAPER_PROTOCOL_METHOD ? ProtocolVersion.getProtocol(Bukkit.getUnsafe().getProtocolVersion()) : ProtocolVersion.getProtocol(HAS_WORLD_VERSION_PROTOCOL_VERSION ? this.cursedProtocolDetection() : this.veryCursedProtocolDetection());
   }
   ...
   protected WrappedChannelInitializer createChannelInitializer(ChannelInitializer oldInitializer) {
   ...
   protected void blame(ChannelHandler bootstrapAcceptor) throws ReflectiveOperationException {
      ...
      if (classLoader.getClass().getName().equals("org.bukkit.plugin.java.PluginClassLoader")) {
         ...
         String var10002 = bootstrapAcceptor.getClass().getName();
         String var6 = description.getName();
         String var5 = var10002;
         throw new RuntimeException("Unable to inject, due to " + var5 + ", try without the plugin " + var6 + "?");
      } else {
         String var8 = bootstrapAcceptor.getClass().getName();
         throw new RuntimeException("Unable to find core component 'childHandler', please check your plugins. issue: " + var8);
      }
   }
