...
public class VelocityPlugin implements ViaServerProxyPlatform {
   ...
   public void onProxyInit(ProxyInitializeEvent e) {
      ...
      this.conf = new VelocityViaConfig(this.configDir.toFile(), this.logger);
   ...
   public ViaCommandSender[] getOnlinePlayers() {
      return (ViaCommandSender[])PROXY.getAllPlayers().stream().map(VelocityCommandSender::new).toArray((x$0) -> new ViaCommandSender[x$0]);
   }
   ...
   public JsonObject getDump() {
      ...
      for(PluginContainer p : PROXY.getPluginManager().getPlugins()) {
         plugins.add(new PluginInfo(true, (String)p.getDescription().getName().orElse(p.getDescription().getId()), (String)p.getDescription().getVersion().orElse("Unknown Version"), (String)p.getInstance().map((instance) -> instance.getClass().getCanonicalName()).orElse("Unknown"), p.getDescription().getAuthors()));
      }
