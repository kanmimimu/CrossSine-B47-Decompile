...
public class EntityMappings1_11 {
   ...
   public static void toClient(CompoundTag tag, boolean backwards) {
      ...
      if (idTag != null) {
         String newName = backwards ? (String)oldToNewNames.inverse().get(idTag.getValue()) : (String)oldToNewNames.get(idTag.getValue());
         if (newName != null) {
   ...
   public static void toServerItem(Item item, boolean backwards) {
      if (hasEntityTag(item)) {
         ...
         if (idTag != null) {
            String newName = backwards ? (String)oldToNewNames.get(idTag.getValue()) : (String)oldToNewNames.inverse().get(idTag.getValue());
            if (newName != null) {
               idTag.setValue(newName);
            }
         }
