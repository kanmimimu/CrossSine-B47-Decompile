...
class ChestConnectionHandler implements ConnectionHandler {
   private static final Int2ObjectMap CHEST_FACINGS = new Int2ObjectOpenHashMap();
   ...
   public int connect(UserConnection user, BlockPosition position, int blockState) {
      ...
      } else if (CHEST_FACINGS.containsKey(relative = this.getBlockData(user, position.getRelative(BlockFace.SOUTH))) && trapped == TRAPPED_CHESTS.contains(relative)) {
         states = (byte)(states | (facing == BlockFace.EAST ? 1 : 2));
      } else if (CHEST_FACINGS.containsKey(relative = this.getBlockData(user, position.getRelative(BlockFace.WEST))) && trapped == TRAPPED_CHESTS.contains(relative)) {
         states = (byte)(states | (facing == BlockFace.NORTH ? 2 : 1));
      } else if (CHEST_FACINGS.containsKey(relative = this.getBlockData(user, position.getRelative(BlockFace.EAST))) && trapped == TRAPPED_CHESTS.contains(relative)) {
