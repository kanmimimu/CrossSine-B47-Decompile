      ...
      if (obfData.isEmpty()) {
         ...
         for(ObfuscationType type : obfData) {
            try {
               ...
               elem.printMessage(this.ap, Kind.ERROR, "Mapping conflict for @Shadow " + elem + ": " + ex.getNew().getSimpleName() + " for target " + target + " conflicts with existing mapping " + ex.getOld().getSimpleName());
            }
         }
