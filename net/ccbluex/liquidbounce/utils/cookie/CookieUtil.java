...
public enum CookieUtil {
   ...
   public LoginData loginWithCookie(File cookieFile) {
      try {
         ...
         JsonObject profileResponse = (JsonObject)this.gson.fromJson(IOUtils.toString(connection.getInputStream()), JsonObject.class);
         return new LoginData(mcToken, (String)null, profileResponse.get("id").getAsString(), profileResponse.get("name").getAsString());
      } catch (Exception e) {
         JOptionPane.showMessageDialog((Component)null, "Failed to login with cookie " + cookieFile.getName().replaceAll(".txt", "."));
         e.printStackTrace();
         return null;
      }
   }
