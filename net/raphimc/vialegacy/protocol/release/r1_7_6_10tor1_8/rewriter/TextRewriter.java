...
public class TextRewriter {
   private static final Object2ObjectMap TRANSLATIONS = new Object2ObjectOpenHashMap(59, 0.99F);
   ...
   public String toClient(UserConnection user, String text) {
      ...
      TextUtils.iterateAll(component, (c) -> {
         AHoverEvent patt22414$temp = c.getStyle().getHoverEvent();
         if (patt22414$temp instanceof TextHoverEvent) {
            TextHoverEvent textHoverEvent = (TextHoverEvent)patt22414$temp;
            if (textHoverEvent.getAction().equals(HoverEventAction.SHOW_ITEM)) {
               try {
                  CompoundTag tag = (CompoundTag)SNbtSerializer.V1_7.deserialize(textHoverEvent.getText().asUnformattedString());
                  ...
                  if (!ID_TO_NAME.containsKey(item.identifier())) {
                     int var10 = item.identifier();
                     throw new IllegalArgumentException("Invalid item ID: " + var10);
                  }

                  tag.putString("id", Key.namespaced((String)ID_TO_NAME.get(item.identifier())));
                  if (damage != item.data()) {
                  ...
                  if (!Via.getConfig().isSuppressConversionWarnings()) {
                     ...
                     String var12 = textHoverEvent.getText().asUnformattedString();
                     var10000.log(var10001, "Error remapping NBT in show_item:" + var12, e);
                  }

                  c.getStyle().setHoverEvent(new TextHoverEvent(textHoverEvent.getAction(), new StringComponent()));
               }
            }
         }
