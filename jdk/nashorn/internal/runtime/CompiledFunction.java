...
final class CompiledFunction {
   ...
   CompiledFunction(MethodHandle invoker, RecompilableScriptFunctionData functionData, Map invalidatedProgramPoints, MethodType callSiteType, int flags) {
   ...
   private synchronized HandleAndAssumptions getValidOptimisticInvocation(Supplier invocationSupplier) {
      while(true) {
   ...
   private static void relinkComposableInvoker(CallSite cs, final CompiledFunction inv, final boolean constructor) {
      HandleAndAssumptions handleAndAssumptions = inv.getValidOptimisticInvocation(new Supplier() {
   ...
   GuardedInvocation createFunctionInvocation(final Class callSiteReturnType, final int callerProgramPoint) {
      return this.getValidOptimisticInvocation(new Supplier() {
         public MethodHandle get() {
   ...
   private static List toStringInvalidations(Map ipp) {
      if (ipp == null) {
   ...
   private void logRecompile(String reason, FunctionNode fn, MethodType type, Map ipp) {
   ...
   private static class OptimismInfo {
      ...
      OptimismInfo(RecompilableScriptFunctionData data, Map invalidatedProgramPoints) {
         this.data = data;
