...
public abstract class ContinuousArrayData extends ArrayData {
   ...
   public final boolean hasRoomFor(int index) {
      return this.has(index) || (long)index == this.length() && this.ensure((long)index) == this;
   }
   ...
   protected MethodHandle getContinuousElementGetter(Class clazz, MethodHandle getHas, Class returnType, int programPoint) {
      ...
      return !isOptimistic ? Lookup.filterReturnType(mh, returnType) : mh;
   }

   protected MethodHandle getContinuousElementSetter(Class clazz, MethodHandle setHas, Class elementType) {
      return Lookup.MH.asType(setHas, setHas.type().changeParameterType(2, elementType).changeParameterType(0, clazz));
   }

   private static final boolean guard(Class clazz, ScriptObject sobj) {
      return sobj != null && sobj.getArray().getClass() == clazz;
   }

   public GuardedInvocation findFastGetIndexMethod(Class clazz, CallSiteDescriptor desc, LinkRequest request) {
      ...
      if (ContinuousArrayData.class.isAssignableFrom(clazz) && indexType == Integer.TYPE) {
         ...
         int index = (Integer)args[args.length - 1];
         if (this.has(index)) {
   ...
   public GuardedInvocation findFastSetIndexMethod(Class clazz, CallSiteDescriptor desc, LinkRequest request) {
      MethodType callType = desc.getMethodType();
      Class<?> indexType = callType.parameterType(1);
   ...
   public ContinuousArrayData fastConcat(ContinuousArrayData otherData) {
      throw new ClassCastException(this.getClass() + " != " + otherData.getClass());
   }

   static {
      FAST_ACCESS_GUARD = Lookup.MH.dropArguments(CompilerConstants.staticCall(MethodHandles.lookup(), ContinuousArrayData.class, "guard", Boolean.TYPE, Class.class, ScriptObject.class).methodHandle(), 2, (Class[])(Integer.TYPE));
   }
}
