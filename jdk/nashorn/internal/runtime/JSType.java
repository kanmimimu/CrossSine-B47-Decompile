...
public enum JSType {
   ...
   private static final List ACCESSOR_TYPES = Collections.unmodifiableList(Arrays.asList(Type.INT, Type.NUMBER, Type.OBJECT));
   ...
   public static final List CONVERT_OBJECT = toUnmodifiableList(TO_INT32.methodHandle(), TO_NUMBER.methodHandle(), null);
   public static final List CONVERT_OBJECT_OPTIMISTIC = toUnmodifiableList(TO_INT32_OPTIMISTIC.methodHandle(), TO_NUMBER_OPTIMISTIC.methodHandle(), null);
   ...
   public static final List GET_UNDEFINED = toUnmodifiableList(Lookup.MH.constant(Integer.TYPE, 0), Lookup.MH.constant(Double.TYPE, Double.NaN), Lookup.MH.constant(Object.class, Undefined.getUndefined()));
   private static final double INT32_LIMIT = (double)4.2949673E9F;
   ...
   public static boolean toBoolean(double num) {
      return num != (double)0.0F && !Double.isNaN(num);
   }
   ...
   public static String toString(double num, int radix) {
      ...
      if (isRepresentableAsInt(num)) {
         ...
         return "NaN";
      } else if (num == (double)0.0F) {
         return "0";
      } else {
         String chars = "0123456789abcdefghijklmnopqrstuvwxyz";
         StringBuilder sb = new StringBuilder();
         boolean negative = num < (double)0.0F;
         ...
         if (decPart > (double)0.0F) {
            ...
            do {
               ...
               decPart -= d;
            } while(decPart > (double)0.0F && sb.length() - dot < 1100);
         }
   ...
   public static int toInt32(long num) {
      return (int)(num >= -9007199254740992L && num <= 9007199254740992L ? num : (long)((double)num % (double)4.2949673E9F));
   }
   ...
   private static long doubleToInt32(double num) {
      ...
      if (exponent < 31) {
         ...
         double d = num >= (double)0.0F ? Math.floor(num) : Math.ceil(num);
         return (long)(d % (double)4.2949673E9F);
      }
   }
   ...
   public static Object toJavaArray(Object obj, Class componentType) {
      if (obj instanceof ScriptObject) {
         ...
         for(int idx = 0; itr.hasNext(); res[idx++] = itr.next()) {
         }
   ...
   public static int decrementExact(int x, int programPoint) throws UnwarrantedOptimismException {
      try {
         ...
         throw new UnwarrantedOptimismException((double)x - (double)1.0F, programPoint);
      }
   }
   ...
   public static int negateExact(int x, int programPoint) throws UnwarrantedOptimismException {
      try {
         if (x == 0) {
            throw new UnwarrantedOptimismException((double)-0.0F, programPoint);
         } else {
   ...
   public static MethodHandle unboxConstant(Object o) {
      if (o != null) {
         if (o.getClass() == Integer.class) {
            return Lookup.MH.constant(Integer.TYPE, (Integer)o);
         }
   ...
   private static final List toUnmodifiableList(MethodHandle... methodHandles) {
