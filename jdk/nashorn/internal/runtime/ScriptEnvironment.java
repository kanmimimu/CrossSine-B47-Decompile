...
public final class ScriptEnvironment {
   ...
   public ScriptEnvironment(Options options, PrintWriter out, PrintWriter err) {
      ...
      if (options.getBoolean("function.statement.error")) {
         this._function_statement = ScriptEnvironment.FunctionStatementBehavior.ERROR;
      } else if (options.getBoolean("function.statement.warning")) {
         this._function_statement = ScriptEnvironment.FunctionStatementBehavior.WARNING;
      } else {
         this._function_statement = ScriptEnvironment.FunctionStatementBehavior.ACCEPT;
      }
      ...
      if (options.get("trace.callsites") instanceof KeyValueOption) {
         ...
         if (kv.hasValue("enterexit") || (callSiteFlags & 256) == 0) {
            callSiteFlags |= 512;
         }
      ...
      this._loggers = (Map)(loggingOption == null ? new HashMap() : loggingOption.getLoggers());
      LoggingOption.LoggerInfo timeLoggerInfo = (LoggingOption.LoggerInfo)this._loggers.get(Timing.getLoggerName());
