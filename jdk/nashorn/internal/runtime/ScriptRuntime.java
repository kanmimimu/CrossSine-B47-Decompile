...
public final class ScriptRuntime {
      ...
      if (xType != yType) {
         ...
         if (xType == JSType.NUMBER) {
            ...
            if (Double.isNaN(xVal) && Double.isNaN(yVal)) {
               return true;
            } else if (xVal == (double)0.0F && Double.doubleToLongBits(xVal) != Double.doubleToLongBits(yVal)) {
               return false;
            } else {
               return xVal == yVal;
            }
   ...
   public static ScriptObject openWith(ScriptObject scope, Object expression) {
      ...
      } else if (expression instanceof ScriptObjectMirror) {
         ...
         if ((!xIsNumber || !yIsUndefined) && (!xIsUndefined || !yIsNumber) && (!xIsUndefined || !yIsUndefined)) {
            ...
            if (!JSType.isString(xPrim) && !JSType.isString(yPrim)) {
               ...
               try {
                  ...
                  throw ECMAErrors.rangeError((Throwable)iae, "concat.string.too.big");
               }
            }
         } else {
            return Double.NaN;
