...
public final class GlobalConstants implements Loggable {
   ...
   private Access getOrCreateSwitchPoint(String name) {
      Access acc = (Access)this.map.get(name);
      if (acc != null) {
         return acc;
      } else {
   ...
   public static MethodHandle staticConstantGetter(Object c) {
      return Lookup.MH.dropArguments(JSType.unboxConstant(c), 0, (Class[])(Object.class));
   }
            ...
            if (!acc.hasBeenInvalidated() && !acc.guardFailed() && !this.invalidatedForever.get()) {
               ...
               if (isOptimistic) {
                  if (JSType.getAccessorTypeIndex(cmh.type().returnType()) <= JSType.getAccessorTypeIndex(retType)) {
                     ...
                     mh = Lookup.MH.dropArguments(Lookup.MH.insertArguments(JSType.THROW_UNWARRANTED.methodHandle(), 0, c, programPoint), 0, (Class[])(Object.class));
                  }
               } else {
               ...
               if (find.getOwner().isGlobal()) {
                  ...
                  guard = Lookup.MH.insertArguments(RECEIVER_GUARD, 0, acc, receiver);
               }
