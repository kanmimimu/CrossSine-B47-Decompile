   ...
   public void shiftLeft(long shift, long length) {
      if (shift != 0L) {
         ...
         if (leftShift == 0) {
            ...
            for(int from = slotShift; slot < slotCount; ++from) {
            ...
            for(int from = slotShift; slot < slotCount; ++slot) {
               long[] var10000 = this.bits;
   ...
   public void shiftRight(long shift, long length) {
      ...
      if (shift != 0L) {
         ...
         if (leftShift == 0) {
            ...
            for(int from = slot - slotShift; slot >= slotShift; this.bits[slot] = this.slot(from)) {
               --slot;
               --from;
            }
         } else {
            ...
            for(int from = slot - slotShift; slot > 0; this.bits[slot] = this.slot(from - 1) >>> leftShift | this.slot(from) << rightShift) {
               --slot;
               --from;
            }
         }
   ...
   public void setRange(long fromIndex, long toIndex) {
      if (fromIndex < toIndex) {
         ...
         if (firstWord == lastWord) {
            ...
            long[] var11 = this.bits;
            var11[firstWord] |= firstBits;
            Arrays.fill(this.bits, firstWord + 1, lastWord, -1L);
            var11 = this.bits;
            var11[lastWord] |= lastBits;
         }
      }
