...
final class Analyser extends Parser {
         ...
         if (type == 5) {
            ...
            if (qn.greedy && QuantifierNode.isRepeatInfinite(qn.upper)) {
               ...
               if (qn.lower <= 1 && qn.target.isSimple()) {
                  n = this.getHeadValueNode(qn.target, false);
                  if (n != null) {
                     Node y = this.getHeadValueNode(nextNode, false);
                     if (y != null && isNotIncluded(n, y)) {
   ...
   private static boolean expandCaseFoldStringAlt(int itemNum, char[] items, char[] chars, int p, int slen, int end, ObjPtr node) {
      ConsAltNode altNode;
   ...
   private Node expandCaseFoldString(Node node) {
      ...
      if (!sn.isAmbig() && sn.length() > 0) {
         ...
         Node xnode = (Node)(topRoot != null ? topRoot : (Node)prevNode.p);
