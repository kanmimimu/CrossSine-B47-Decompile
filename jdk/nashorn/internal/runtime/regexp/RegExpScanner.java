...
final class RegExpScanner extends Scanner {
   ...
   private final Map expected = new HashMap();
   ...
   BitVector getGroupsInNegativeLookahead() {
      ...
      for(int i = 0; i < this.caps.size(); ++i) {
         Capture cap = (Capture)this.caps.get(i);
         if (cap.negLookaheadLevel > 0) {
            if (vec == null) {
               vec = new BitVector((long)(this.caps.size() + 1));
            }
   ...
   private void push(char ch) {
      this.expected.put(ch, (Integer)this.expected.get(ch) + 1);
   }

   private void pop(char ch) {
      this.expected.put(ch, Math.min(0, (Integer)this.expected.get(ch) - 1));
   }
   ...
   private boolean patternCharacter() {
      if (this.atEOF()) {
         ...
         switch (this.ch0) {
            ...
            case ']':
            case '}':
               int n = (Integer)this.expected.get(this.ch0);
               if (n != 0) {
                  return false;
               }
            case '{':
      ...
      if (this.ch0 == '0' && !isOctalDigit(this.ch1)) {
         ...
         if (this.ch0 == '0') {
            ...
            if (this.inCharClass) {
               ...
               Capture capture = (Capture)this.caps.get(decimalValue - 1);
