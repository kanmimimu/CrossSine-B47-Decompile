...
final class Splitter extends SimpleNodeVisitor implements Loggable {
   ...
   FunctionNode split(FunctionNode fn, boolean top) {
      ...
      if (weight >= SPLIT_THRESHOLD) {
         ...
         if (weight >= SPLIT_THRESHOLD) {
            FunctionNode var9 = functionNode.setBody((LexicalContext)null, this.splitBlock(functionNode.getBody(), functionNode));
            functionNode = var9.setFlag((LexicalContext)null, 16);
   ...
   private static List directChildren(final FunctionNode functionNode) {
   ...
   private SplitNode createBlockSplitNode(Block parent, FunctionNode function, List statements, long weight) {
