...
public final class ObjectClassGenerator implements Loggable {
   ...
   private void initializeToUndefined(MethodEmitter init, String className, List fieldNames) {
      if (!this.dualFields) {
         if (!fieldNames.isEmpty()) {
            ...
            String fieldName;
            for(Iterator<String> iter = fieldNames.iterator(); iter.hasNext(); init.putField(className, fieldName, Type.OBJECT.getDescriptor())) {
   ...
   private List addFields(ClassEmitter classEmitter, int fieldCount) {
   ...
   private static MethodHandle createGetterInner(Class forType, Class type, MethodHandle primitiveGetter, MethodHandle objectGetter, List converters, int programPoint) {
   ...
   private static Class getBoxedType(Class forType) {
