...
final class CodeGenerator extends NodeOperatorVisitor implements Loggable {
   ...
   private final Deque scopeEntryLabels = new ArrayDeque();
   private static final Label METHOD_BOUNDARY;
   private final Deque catchLabels = new ArrayDeque();
      ...
      if (!symbol.isScope()) {
         ...
         if (this.isFastScope(symbol)) {
            if (symbol.getUseCount() > 200 && !identNode.isOptimistic()) {
               (new OptimisticOperation(identNode, CodeGenerator.TypeBounds.OBJECT) {
                  void loadStack() {
   ...
   private MethodEmitter loadBinaryOperands(Expression lhs, Expression rhs, TypeBounds explicitOperandBounds, boolean baseAlreadyOnStack, boolean forceConversionSeparation) {
      ...
      if (!noToPrimitiveConversion(lhsType, explicitOperandBounds.widest) && !rhs.isLocal()) {
         TypeBounds safeConvertBounds = CodeGenerator.TypeBounds.UNBOUNDED.notNarrowerThan(narrowestOperandType);
   ...
   private int loadArgs(List args) {
   ...
   private void enterForIn(final ForNode forNode) {
      ...
      (new Store(forNode.getInit()) {
         protected void storeNonDiscard() {
         }

         protected void evaluate() {
            (new OptimisticOperation((Optimistic)forNode.getInit(), CodeGenerator.TypeBounds.UNBOUNDED) {
               void loadStack() {
   ...
   private void initLocals(Block block) {
      ...
      if (block.needsScope()) {
         ...
         (new FieldObjectCreator(this, tuples, true, hasArguments) {
   ...
   private MethodEmitter loadArgsArray(List args) {
   ...
   private boolean undefinedCheck(RuntimeNode runtimeNode, List args) {
   ...
   private boolean nullCheck(RuntimeNode runtimeNode, List args) {
      RuntimeNode.Request request = runtimeNode.getRequest();
   ...
   private boolean isDeoptimizedExpression(final Expression rootExpr) {
      return !this.isRestOf() ? false : (new Supplier() {
         boolean contains;
   ...
   private void loadSplitLiteral(SplitLiteralCreator creator, List ranges, Type literalType) {
      assert ranges != null;
   ...
   private void loadDECINC(final UnaryNode unaryNode) {
      ...
      (new SelfModifyingStore(unaryNode, operand) {
         ...
         private void loadMinusOne() {
            if (type.isInteger()) {
               ...
               CodeGenerator.this.method.load(isIncrement ? (double)1.0F : (double)-1.0F);
            }
   ...
   private void loadASSIGN_SHR(final BinaryNode binaryNode) {
      (new SelfModifyingStore(binaryNode, binaryNode.lhs()) {
         protected void evaluate() {
   ...
   private String getLvarTypesDescriptor(List localVarTypes) {
   ...
   private void generateContinuationHandler() {
      if (this.isRestOf()) {
         ...
         nextLvarIndex = stackStoreSpec.length == 0;
   ...
   private static final class TypeBounds {
      ...
      TypeBounds maybeNew(Type newNarrowest, Type newWidest) {
         return newNarrowest == this.narrowest && newWidest == this.widest ? this : new TypeBounds(newNarrowest, newWidest);
      }
   ...
   private abstract class SelfModifyingStore extends Store {
      protected SelfModifyingStore(Expression assignNode, Expression target) {
   ...
   private abstract class Store {
      ...
      protected Store(Expression assignNode, Expression target) {
         this.assignNode = assignNode;
         this.target = target;
      }

      protected Store(Expression assignNode) {
         this(assignNode, assignNode);
      }
   ...
   private abstract class OptimisticOperation {
      ...
      private void addUnwarrantedOptimismHandlerLabel(List localTypes, Label label) {
