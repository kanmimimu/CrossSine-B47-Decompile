...
public class MethodEmitter {
   ...
   private final Map localVariableDefs;
   ...
   void _catch(Collection recoveries) {
      assert this.stack == null;
   ...
   MethodEmitter loadHandle(String className, String methodName, String descName, EnumSet flags) {
   ...
   void closeLocalVariable(Symbol symbol, Label label) {
      LocalVariableDef def = (LocalVariableDef)this.localVariableDefs.get(symbol);
      if (def != null) {
   ...
   void store(Symbol symbol, Type type, boolean onlySymbolLiveValue) {
      ...
      if (symbol.hasSlot()) {
         ...
         LocalVariableDef existingDef = (LocalVariableDef)this.localVariableDefs.get(symbol);
   ...
   List getLocalVariableTypes() {
      return this.stack.localVariableTypes;
   }

   List getWidestLiveLocals(List localTypes) {
   ...
   private void debug(int padConstant, Object... args) {
      if (this.debug) {
         ...
         for(int var10 = padConstant - sb.length(); var10 > 0; --var10) {
            sb.append(' ');
         }
