...
public class ClassEmitter {
   ...
   private void defineCommonUtilities() {
      ...
      if (this.constantMethodNeeded.contains(String.class)) {
         MethodEmitter getStringMethod = this.method(EnumSet.of(ClassEmitter.Flag.PRIVATE, ClassEmitter.Flag.STATIC), CompilerConstants.GET_STRING.symbolName(), String.class, Integer.TYPE);
      ...
      if (this.constantMethodNeeded.contains(PropertyMap.class)) {
         MethodEmitter getMapMethod = this.method(EnumSet.of(ClassEmitter.Flag.PUBLIC, ClassEmitter.Flag.STATIC), CompilerConstants.GET_MAP.symbolName(), PropertyMap.class, Integer.TYPE);
         getMapMethod.begin();
   ...
   static String disassemble(byte[] bytecode) {
      ...
      try {
         ...
         Context ctx = (Context)AccessController.doPrivileged(new PrivilegedAction() {
            public Context run() {
   ...
   MethodEmitter method(EnumSet methodFlags, String methodName, Class rtype, Class... ptypes) {
      ++this.methodCount;
   ...
   MethodEmitter method(EnumSet methodFlags, String methodName, String descriptor) {
      ++this.methodCount;
      this.methodNames.add(methodName);
   ...
   MethodEmitter init(EnumSet flags, Class... ptypes) {
      ++this.initCount;
      return this.method(flags, CompilerConstants.INIT.symbolName(), Void.TYPE, ptypes);
   }

   final void field(EnumSet fieldFlags, String fieldName, Class fieldType, Object value) {
      ++this.fieldCount;
   ...
   final void field(EnumSet fieldFlags, String fieldName, Class fieldType) {
   ...
   private MethodVisitor methodVisitor(EnumSet flags, String methodName, Class rtype, Class... ptypes) {
   ...
   static enum Flag {
      ...
      static int getValue(EnumSet flags) {
         int v = 0;
