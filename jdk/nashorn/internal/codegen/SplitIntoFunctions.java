...
final class SplitIntoFunctions extends NodeVisitor {
   ...
   private final Deque functionStates = new ArrayDeque();
   private final Deque splitStates = new ArrayDeque();
   ...
   public Node leaveSplitNode(SplitNode splitNode) {
      ...
      if (hasReturn && fnState.splitDepth > 0) {
         SplitState parentSplit = (SplitState)this.splitStates.peek();
         if (parentSplit != null) {
      ...
      if (splitState.hasBreak) {
         splitStateHandler = makeIfStateEquals(firstLineNumber, token, finish, 1, this.enblockAndVisit(new BreakNode(-1, token, finish, (String)null)), splitStateHandler);
      }
   ...
   private static void addCase(List cases, int i, Block body) {
