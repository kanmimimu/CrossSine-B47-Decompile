...
public class Block extends Node implements BreakableNode, Terminal, Flags {
   ...
   public Block(long token, int finish, List statements) {
      this(token, finish, (Statement[])statements.toArray(new Statement[statements.size()]));
   }

   private Block(Block block, int finish, List statements, int flags, Map symbols, LocalVariableConversion conversion) {
      super(block);
   ...
   public Block replaceSymbols(LexicalContext lc, Map replacements) {
      if (this.symbols.isEmpty()) {
         return this;
      } else {
         LinkedHashMap<String, Symbol> newSymbols = new LinkedHashMap(this.symbols);

         for(Map.Entry entry : newSymbols.entrySet()) {
   ...
   public Block setStatements(LexicalContext lc, List statements) {
