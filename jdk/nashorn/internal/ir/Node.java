...
public abstract class Node implements Cloneable, Serializable {
   ...
   static List accept(NodeVisitor visitor, List list) {
      int size = list.size();
      if (size == 0) {
         return list;
      } else {
   ...
   static LexicalContextNode replaceInLexicalContext(LexicalContext lc, LexicalContextNode oldNode, LexicalContextNode newNode) {
      if (lc != null) {
