...
public final class ObjectSizeCalculator {
   ...
   private ClassSizeInfo getClassSizeInfo(Class clazz) {
      ClassSizeInfo csi = (ClassSizeInfo)this.classSizeInfos.get(clazz);
      if (csi == null) {
      ...
      if (componentType.isPrimitive()) {
         ...
         switch (length) {
            ...
            case 1:
               ...
               break;
            default:
               this.enqueue(new ArrayElementsVisitor(array));
         }
      }
      ...
      if (vmName != null && vmName.startsWith("Java HotSpot(TM) ")) {
         ...
         if ("32".equals(dataModel)) {
            ...
            if (vmVersion >= 17) {
               ...
               try {
                  for(Object mp : (List)getMemoryPoolMXBeans.invoke(managementFactory)) {
                     Object usage = getUsage.invoke(mp);
                     Object max = getMax.invoke(usage);
                     maxMemory += (Long)max;
                  }
