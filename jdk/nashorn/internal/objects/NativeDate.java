...
public final class NativeDate extends ScriptObject {
   ...
   private static InvokeByName getTO_ISO_STRING() {
      return Global.instance().getInvokeByName(TO_ISO_STRING, new Callable() {
      ...
      if (!isNew) {
         ...
         switch (args.length) {
            ...
            case 1:
               ...
               break;
            default:
               result = new NativeDate((double)0.0F);
   ...
   public static double UTC(Object self, Object... args) {
      NativeDate nd = new NativeDate((double)0.0F);
      double[] d = convertCtorArgs(args);
   ...
   public static double setFullYear(Object self, Object... args) {
      ...
      if (nd.isValidDate()) {
         ...
         double[] d = convertArgs(args, (double)0.0F, 0, 0, 3);
         if (d != null) {
            nd.setTime(timeClip(utc(makeDate(makeDay(d[0], d[1], d[2]), (double)0.0F), nd.getTimeZone())));
         } else {
   ...
   public static double setUTCFullYear(Object self, Object... args) {
      ...
      if (nd.isValidDate()) {
         ...
         double[] d = convertArgs(args, (double)0.0F, 0, 0, 3);
         nd.setTime(timeClip(makeDate(makeDay(d[0], d[1], d[2]), (double)0.0F)));
      }
   ...
   public static double setYear(Object self, Object year) {
      ...
      if (Double.isNaN(nd.getTime())) {
         nd.setTime(utc((double)0.0F, nd.getTimeZone()));
      }
   ...
   private static double timeWithinDay(double t) {
      double val = t % (double)8.64E7F;
      return val < (double)0.0F ? val + (double)8.64E7F : val;
   }
   ...
   private static double timeFromYear(int y) {
      return dayFromYear((double)y) * (double)8.64E7F;
   }
   ...
   private static int yearFromTime(double t) {
      ...
      if (t2 > t) {
         --y;
      } else if (t2 + (double)8.64E7F * (double)daysInYear(y) <= t) {
         ++y;
      }
   ...
   private static int weekDay(double time) {
      int day = (int)(day(time) + (double)4.0F) % 7;
   ...
   private static int hourFromTime(double t) {
      int h = (int)(Math.floor(t / (double)3600000.0F) % (double)24.0F);
      return h < 0 ? h + 24 : h;
   }

   private static int minFromTime(double t) {
      int m = (int)(Math.floor(t / (double)60000.0F) % (double)60.0F);
      return m < 0 ? m + 60 : m;
   }

   private static int secFromTime(double t) {
      int s = (int)(Math.floor(t / (double)1000.0F) % (double)60.0F);
      return s < 0 ? s + 60 : s;
   }

   private static int msFromTime(double t) {
      int m = (int)(t % (double)1000.0F);
      return m < 0 ? m + 1000 : m;
   }
   ...
   private static double makeTime(double hour, double min, double sec, double ms) {
      return hour * (double)3600000.0F + min * (double)60000.0F + sec * (double)1000.0F + ms;
   }

   private static double makeDay(double year, double month, double date) {
      double y = year + Math.floor(month / (double)12.0F);
      int m = (int)(month % (double)12.0F);
      if (m < 0) {
         m += 12;
      }
   ...
   private static double makeDate(Integer[] d) {
      double time = makeDay((double)d[0], (double)d[1], (double)d[2]) * (double)8.64E7F;
      return time + makeTime((double)d[3], (double)d[4], (double)d[5], (double)d[6]);
   }

   private static double makeDate(double[] d) {
      double time = makeDay(d[0], d[1], d[2]) * (double)8.64E7F;
   ...
   private static double[] convertCtorArgs(Object[] args) {
      ...
      for(int i = 0; i < d.length; ++i) {
         if (i < args.length) {
            ...
            d[i] = i == 2 ? (double)1.0F : (double)0.0F;
         }
      }

      if ((double)0.0F <= d[0] && d[0] <= (double)99.0F) {
         d[0] += (double)1900.0F;
      }
   ...
   private static double timeClip(double time) {
      return !Double.isInfinite(time) && !Double.isNaN(time) && !(Math.abs(time) > 8.64E15) ? (double)((long)time) : Double.NaN;
   }
