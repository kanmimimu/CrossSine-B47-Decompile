...
class MaximallySpecific {
   private static final MethodTypeGetter METHOD_HANDLE_TYPE_GETTER = new MethodTypeGetter() {
      MethodType getMethodType(MethodHandle t) {
         return t.type();
      }
   };
   private static final MethodTypeGetter DYNAMIC_METHOD_TYPE_GETTER = new MethodTypeGetter() {
   ...
   static List getMaximallySpecificMethodHandles(List methods, boolean varArgs, Class[] argTypes, LinkerServices ls) {
      return getMaximallySpecificMethods(methods, varArgs, argTypes, ls, METHOD_HANDLE_TYPE_GETTER);
   }

   static List getMaximallySpecificSingleDynamicMethods(List methods, boolean varArgs, Class[] argTypes, LinkerServices ls) {
      return getMaximallySpecificMethods(methods, varArgs, argTypes, ls, DYNAMIC_METHOD_TYPE_GETTER);
   }

   private static List getMaximallySpecificMethods(List methods, boolean varArgs, Class[] argTypes, LinkerServices ls, MethodTypeGetter methodTypeGetter) {
      if (methods.size() < 2) {
         return methods;
      } else {
   ...
   private abstract static class MethodTypeGetter {
      ...
      abstract MethodType getMethodType(Object var1);
   }
}
